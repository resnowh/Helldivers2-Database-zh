...
| 字段 | 含义 | 实战意义 |
| --- | --- | --- |
| `部位血量（Health）` | 部位血量 | 决定该部位本身要承受多少伤害才会破坏。 |
| `装甲等级（AV）` | 装甲等级 | 武器穿甲不足时，面板伤害再高也可能打不进去。 |
| `耐久比例（Durable）` | 耐久比例 | 对高耐久部位时，更应参考武器的耐久伤害而不是标准伤害。 |
| `转伤主生命比例（% To Main）` | 转伤主生命比例 | 说明打该部位时，有多少伤害会同步结算到主生命。 |
| `溢出封顶（Overflow Cap）` | 溢出封顶 | 若为 `Yes`，转给主生命的伤害会受到该部位最大血量限制。 |
| `致命判定（Fatal）` | 是否致命 | 有些部位被摧毁后可直接击杀目标。 |
| `肢体生命/衰亡池（Constitution）` | 额外肢体生命/衰亡池 | 某些肢体在主部位打空后还会继续维持一段时间，或开始衰亡。 |
| `爆炸伤害抗性（ExDR）` | 爆炸伤害抗性 | 决定爆炸伤害是否有效，以及能否靠爆炸绕过部位处理主血。 |

#### 通用结论

- `高转伤 + 可穿透 + 可稳定命中` 的部位，才是真正高价值弱点。
- `无甲软部位` 不一定就最好打；若其 `耐久比例（Durable）` 很高而武器耐久伤害很低，效率仍然可能很差。
