...
- `Anatomy` 表中的核心字段包括：`Health`、`AV`、`Durable`、`% To Main`、`Overflow Cap`、`Fatal`、`Constitution`、`ExDR`。
...
| 字段 | 含义 | 实战意义 |
| --- | --- | --- |
| `Health` | 部位血量 | 决定该部位本身要承受多少伤害才会破坏。 |
| `AV` | Armor Value，装甲等级 | 武器穿甲不足时，面板伤害再高也可能打不进去。 |
| `Durable` | 耐久比例 | 对高耐久部位时，更应参考武器的耐久伤害而不是标准伤害。 |
| `% To Main` | 转伤主生命比例 | 说明打该部位时，有多少伤害会同步结算到主生命。 |
| `Overflow Cap` | 溢出封顶 | 若为 `Yes`，转给主生命的伤害会受到该部位最大血量限制。 |
| `Fatal` | 是否致命 | 有些部位被摧毁后可直接击杀目标。 |
| `Constitution` | 额外肢体生命/衰亡池 | 某些肢体在主部位打空后还会继续维持一段时间，或开始衰亡。 |
| `ExDR` | 爆炸伤害抗性 | 决定爆炸伤害是否有效，以及能否靠爆炸绕过部位处理主血。 |

#### 通用结论

- `高转伤 + 可穿透 + 可稳定命中` 的部位，才是真正高价值弱点。
- `无甲软部位` 不一定就最好打；若其 `Durable` 很高而武器耐久伤害很低，效率仍然可能很差。
- `Fatal = Yes` 的部位价值极高，但前提仍然是穿得进去、打得准。
- `Constitution` 常见于大型单位的腿部或肢体，这也是某些目标“看起来腿断了却没立刻死”的原因。
...
- 机器人中甲步兵的核心 Anatomy 规律就是：`正面躯干能打，但不值得；头部更值。`
...
| Commando（常用译名：突击兵） | MLS-4X Commando | 四联制导/直射导弹 | 1100 投射物 + 150 爆炸 | 1100 + 150 | `Anti-Tank II` | 单发低于 EAT / Quasar，但一筒四发，容错高。 |
...
| 爆裂铳 | R-36 Eruptor | 爆炸步枪 | 230 投射物 + 225 爆炸 | 页面待补 | `Heavy投射物 / Medium爆炸` | 主武器里最接近“轻型反甲”的选择之一。 |
...
| Commando（常用译名：突击兵） | MLS-4X Commando | 一筒四发，手持即用 | 低中 | 多发容错型反重，适合连续拆部件。 |
...
| 目标 | 主血/关键数据 | 飞矛 | 无后坐力炮 | 消耗性反坦克武器 / 类星体加农炮 | Commando | 磁轨炮 | 反器材步枪 / 机炮 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| 多只重甲连续刷新 | 无后坐力炮、消耗性反坦克武器 | 类星体加农炮、Commando | 只带单发高爆且缺续航 |
| 机器人弱点拆解 | 反器材步枪、机炮、磁轨炮 | 重机枪 | 纯火焰流 |
| 光能者拆腿/拆盾发生器 | 机炮、重机枪、激光大炮 | 磁轨炮、Commando | 只打护盾表面 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 高爆手雷 | G-12 High Explosive | 800 爆炸 | `Heavy` | 3.5 秒，可预热 | 4 | 面板最高的常规爆炸手雷，兼顾反甲与炸建筑。 |
| 破片手雷 | G-6 Frag | 500 爆炸 + 110 破片 x35 | `Medium` | 2.4 秒，可预热 | 6 | 大范围清群，反人群优于反重。 |
| 冲击手雷 | G-16 Impact | 400 爆炸 | `Heavy` | 碰撞起爆 | 4 | 即时起爆，适合快速补刀和瞬间炸洞。 |
| 燃烧手雷 | G-10 Incendiary | 300 爆炸 + 100 火焰/秒 | 爆炸 `Medium`；火焰 `Heavy` | 2.9 秒，可预热 | 4 | 堵口、守路、烧终结族和无票者非常强。 |
| 燃烧冲击手雷 | G-13 Incendiary Impact | 150 爆炸 + 100 火焰/秒 | 爆炸 `Medium`；火焰 `Heavy` | 碰撞起爆 | 4 | 更适合中近距离立刻点燃小群敌人。 |
| 毒气手雷 | G-4 Gas | 3 爆炸 + 25 毒气/秒 + 混乱 | `Anti-Tank II` | 2.9 秒，可预热 | 4 | 控场型手雷，可致盲、减速、打乱路径。 |
| 眩晕手雷 | G-23 Stun | 0 伤害；5 秒眩晕 | `Anti-Tank II` | 1.8 秒，可预热 | 4 | 最强控制手雷之一，专门给弱点输出和战略配备创造窗口。 |
| 铝热手雷 | G-123 Thermite | 150 火焰/秒 x 6.5 秒 + 2000 爆炸 | `Tank III` | 2.9 秒，可预热；黏附 | 3 | 手雷里的纯反重首选，对单体重甲最强。 |
| 追猎手雷 | G-50 Seeker | 500 爆炸 | `Heavy` | 自动索敌后碰撞起爆 | 4 | 会追目标，适合打移动目标和高角度弱点。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 机枪哨戒炮 | A/MG-43 Machine Gun Sentry | 80 弹道 | `Light` | 150 | 90 秒 | 最基础反群哨戒炮，主要负责清杂而非反重。 |
| 加特林哨戒炮 | A/G-16 Gatling Sentry | 90 弹道 | `Medium` | 500 | 150 秒 | 高射速压制，适合清轻中型目标，但不该当主反重。 |
| 机炮哨戒炮 | A/AC-8 Autocannon Sentry | 450 弹道 + 150 爆炸 | `Anti-Tank I` | 24，升级后 36 | 150 秒 | 三连发高爆反甲壳，对单体重目标 DPS 极高。 |
| 火箭哨戒炮 | A/MLS-4X Rocket Sentry | 单发火箭伤害高于机炮单发；爆炸为中穿 | `Anti-Tank I` | 页面主面板已确认，细分弹量待补 | 150 秒 | 超远射程，偏重型目标优先，持续反重能力强。 |
...
| 中文名 | 英文名 | 标准伤害 | 穿甲 | 供弹 | 冷却 | 关键特征 |
| --- | --- | --- | --- | --- | --- | --- |
| 重机枪炮座 | E/MG-101 HMG Emplacement | 200 弹道；耐久伤害 40 | `Heavy` | 300 | 180 秒 | 人工控制，持续火力极强，适合守线与拆中重目标部件。 |
| 反坦克炮座 | E/AT-12 Anti-Tank Emplacement | 1300 弹道 + 150 爆炸；耐久伤害同 1300 | `Anti-Tank II` | 30 | 180 秒 | 人工控制的高速反坦克炮，能稳定处理绝大多数重甲。 |
| 解放者机甲 | EXO-49 Emancipator Exosuit | 双机炮单发 300；耐久伤害 150；爆炸 150 | `Anti-Tank I` | 每臂 100 发 | 600 秒 | 持续拆件、拆腿和处理中重混编目标的综合能力最强。 |
...
| --- | --- | --- | --- | --- |
| Terminid Bug Hole | 虫洞 / 虫巢洞口 | 可以 | 高爆手雷、冲击手雷、燃烧手雷、榴弹发射器、爆炸类主武器 | 冲击手雷更吃落点；定时爆炸物容错更高。 |
| Automaton Fabricator | 机器人工厂 / 兵工厂 | 可以 | 高爆手雷、冲击手雷、榴弹发射器、机炮、反坦克武器 | 角度与开口位置比纯伤害更重要。 |
| Illuminate Warp Ship | 跃迁飞船 | 不建议依赖手雷 | 无后坐力炮、类星体加农炮、飞矛、轨道精准攻击、飞鹰500KG炸弹 | 通常先处理护盾，再处理船体。 |
| Automaton Cannon Turret | 炮塔 / 加农炮塔 | 不推荐手雷 | 反器材步枪、机炮、无后坐力炮、轨道精准攻击 | 后部弱点是高收益命中区。 |
| Detector Tower | 探测塔 | 不推荐手雷 | 轨道精准攻击、500KG 炸弹、反坦克武器 | 优先级高，属于典型应尽快清除的据点目标。 |
| Stratagem Jammer | 战略配备干扰器 | 不能简单依赖手雷 | 终端解除或重火力拆毁关联结构 | 与其他据点不同，处理方式更依赖任务结构。 |
| SEAF SAM Site | 超级地球武装部队地对空导弹基地 | 不属于破坏类目标 | 激活为主 | 中文页已明确该目标只会出现在机器人或光能者星球。 |
| SEAF Artillery | 超级地球武装部队大炮 | 不属于破坏类目标 | 激活与装填为主 | 属于可解锁支援战略配备的可选目标。 |
...
| 巨型者 | 头部、背散热器 | 不推荐主武器当主解 | 反器材步枪、磁轨炮、无后坐力炮、机炮 | 铝热手雷、眩晕手雷 | 轨道炮攻击、飞鹰500KG炸弹 | `AMR / Railgun 一枪头` 是最节省资源的打法。 |
